...
Matrix multiply(const Matrix& a, const Matrix& b_transposed, int n) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const Matrix c = multiply(a, b_transposed, n_a);
        ...
        std::cout << "size=" << n_a << '\n';
        std::cout << "elements=" << static_cast<long long>(n_a) * n_a << '\n';
        std::cout << "operations=" << std::fixed << std::setprecision(0) << operations << '\n';
        std::cout << "time_sec=" << std::setprecision(6) << elapsed.count() << '\n';
