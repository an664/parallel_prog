...
Защита работы может опираться на `measured_results.csv`, полученный после
запуска `submit_job.sh` на суперкомпьютере. Подготовленный репозиторий содержит
все файлы, необходимые для такого воспроизведения.
