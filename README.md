...
| `LAB_5` | MPI на суперкомпьютере | [LAB_5/README.md](./LAB_5/README.md) |
...
Отчет по [лабораторной работе №5](./LAB_5/README.md) содержит код и Slurm-скрипты
для запуска MPI-версии на суперкомпьютере.

## Общий формат данных

В лабораторных работах №1-4 входной файл содержит размерность `N`, затем `N`
строк по `N` чисел:
...
матрицы, далее идет матрица `C = A * B`. В лабораторной работе №5 матрицы
генерируются детерминированно в памяти, чтобы не копировать большие входные
файлы на суперкомпьютер.

## Верификация

Для лабораторных работ №1-4 есть скрипт `verify.py`, который вычисляет эталонный
результат через NumPy и сравнивает его с результатом C/C++/OpenCL программы
через `numpy.allclose`.
